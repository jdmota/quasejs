...
export class BuilderPack {
  ...
  private async writeAsset(
    ...
    if (map) {
      map.sources = map.sources.map((source: string) =>
        relative(resolvePath(source, this.options.cwd), directory)
      );
    }
  ...
  private async render(
    ...
    const { result } = await this.builder.pluginsRunner.renderAsset(
      asset,
      hashIds,
      new BuilderUtil(this.options)
    );
  ...
  private shouldRebuild(
    ...
    const oldInlineAssets = new Map(
      oldFile.inlineAssets.map(a => [a.module.id, a])
    );
    for (const a of newFile.inlineAssets) {
      if (
        this.shouldRebuild(processedGraph, oldInlineAssets.get(a.module.id), a)
      ) {
        return true;
      }
    }
  ...
  async run(processedGraph: ProcessedGraph) {
    ...
    const filesToRender = processedGraph.files.filter(f =>
      this.shouldRebuild(processedGraph, this.previousFiles.get(f.module.id), f)
    );

    // Render them
    const filesInfo = await Promise.all(
      filesToRender.map(a =>
        this.render(a, processedGraph.hashIds, currWrittenFiles)
      )
    );
    ...
    for (const asset of processedGraph.files) {
      const files =
        currWrittenFiles.get(asset.module.id) ||
        this.allWrittenFiles.get(asset.module.id);
    ...
    this.previousFiles = new Map(
      processedGraph.files.map(v => [v.module.id, v])
    );
