...
function defaultTasks(providedTasks: any = {}) {
  ...
  for (const key in tasks) {
    ...
    if (
      providedTasks[key] === true ||
      providedTasks[key] === "true" ||
      providedTasks[key] === undefined
    ) {
      // @ts-ignore
      ...
      throw error(
        `Expected task '${key}' to be a function, instead saw ${providedTasks[key]}`
      );
    }
  }
  return tasksToRun;
...
async function handleOptions(
  ...
  const rootPkgJob: Promise<Pkg> | undefined =
    folder === cwd ? undefined : readPkg({ cwd: cwd });

  const pkg = await pkgJob;
  const rootPkg = await rootPkgJob;

  const pkgPath: string = slash(path.resolve(folder, "package.json"));
  const pkgNodeModules: string = slash(path.resolve(folder, "node_modules"));
  ...
  if (!isValidVersion(pkg.version)) {
    throw error(
      `Missing valid 'version' on ${pkgRelativePath}. If this is the first version, just set it to 0.0.1, e.g.`
    );
  }
  ...
  const d = <T>(value: T | undefined, defaultt: T) =>
    value === undefined ? defaultt : value;
  ...
  if (provided.git !== false) {
    const gitMessageJob = git.commitAndTag
      ? git.message || getVersionGitMessage(yarn)
      : undefined;
    const gitTagPrefixJob =
      git.commitAndTag === true
        ? git.tagPrefix || getVersionTagPrefix(yarn)
        : undefined;
    const gitRootJob: Promise<string> = execStdout("git", [
      "rev-parse",
      "--show-toplevel",
    ]);
  ...
  if (git.commitAndTag === true && !git.tagPrefix) {
    throw error(
      `Empty git tag prefix. Set git.commitAndTag to 'only-commit' if you do not want to tag the commit.`
    );
  }
...
export async function publish(
  ...
  if (opts.git.message) {
    ...
    info(
      `Git Message: ${git.createMessage(
        opts as Options & { git: GitOptions & { message: string } }
      )}`
    );
    if (opts.git.tagPrefix) {
      info(
        `Git Tag: ${git.createTag(
          opts as Options & { git: GitOptions & { tagPrefix: string } }
        )}`
      );
    }
  }
