    ...
    task(_ctx: any, task: any) {
      return execStdout(opts.yarn ? "yarn" : "npm", ["audit"], {
        ...
        if (/(Did you mean|npm <command>)/.test(err.stdout)) {
          task.skip(
            "This version of npm does not support audit. Upgrade to npm>=6"
          );
          return;
        }
        if (/Command "audit" not found/.test(err.stderr)) {
          task.skip(
            "This version of yarn does not support audit. Upgrade to yarn>=1.12"
          );
          return;
        }
        err.message = "\n" + err.all;
...
export function cleanup(opts: Options) {
  return [
      ...
      task: () => {
        if (!hasYarn(opts.folder)) {
          return Promise.reject(
            error("Cannot use Yarn without yarn.lock file")
          );
        }
    ...
    },
    {
      ...
      enabled: () => !opts.yarn,
      task: () => {
        const args = opts.hasNpmLockfile
          ? ["ci"]
...
function buildRootLifecycle(arr: string[], opts: Options) {
  return arr.map(name => {
    return {
      title: `Executing root's ${name} script using ${
        opts.yarn ? "Yarn" : "npm"
      }`,
      enabled: () =>
        !!(opts.rootPkg && opts.rootPkg.scripts && opts.rootPkg.scripts[name]),
      task: () =>
        execObservable(
          opts.yarn ? "yarn" : "npm",
          [opts.yarn ? "run" : "run-script", name],
          {
            cwd: opts.cwd,
            history: opts.history,
          }
...
export function bumpVersion(opts: Options) {
  if (opts.yarn) {
    return {
      ...
      task: () =>
        execObservable(
          "yarn",
          ["version", "--new-version", opts.version, "--no-git-tag-version"],
          {
            cwd: opts.folder,
            history: opts.history,
          }
  ...
  return {
    ...
    task: () =>
      execObservable("npm", ["version", opts.version, "--no-git-tag-version"], {
        cwd: opts.folder,
        history: opts.history,
      }),
  };
...
export function commitAndTag(opts: Options) {
  return {
    ...
    task: async () => {
      ...
      const message = git.createMessage(
        opts as Options & { git: GitOptions & { message: string } }
      );
      const tag = git.createTag(
      ...
      if (opts.git.commitAndTag !== "only-commit") {
        await execPromise(
          "git",
          ["tag", tag, opts.git.signTag ? "-sm" : "-am", message],
          execOpts
        );
      }
    },
...
export function gitPush(opts: Options) {
  return {
    title:
      opts.git.commitAndTag === "only-commit"
        ? "Pushing commit"
    ...
    task: () =>
      git.push(opts as Options & { git: GitOptions & { push: true } }),
  };
}
