...
currency.prototype = {
  ...
  distribute2(count) {
    ...
    const leftOvers = currency(1 / precision).multiply(pennies);

    return { distribution, pennies, leftOvers };
  },

  /**
   * All pay extra
   */
  distribute3(count, except) {
    let { intValue, _precision, _settings } = this,
      distribution = [],
      split = Math[intValue >= 0 ? "floor" : "ceil"](intValue / count),
      pennies = Math.abs(intValue - split * count),
   ...
   */
  compareTo(other) {
    const diff = this.dollars() - other.dollars();
    return diff === 0 ? this.cents() - other.cents() : diff;
  },
