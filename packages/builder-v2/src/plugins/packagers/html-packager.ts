...
class HtmlRenderer {
  ...
  createSrcScript(src: string, noDefer?: boolean) {
    return this.treeAdapter.createElement(
      "script",
      NAMESPACE,
      [
  ...
  async render(
    ...
    if (asset.runtime.manifest) {
      const code = `${chunkInit}.p({},${JSON.stringify(
        asset.runtime.manifest
      )})`;
      this.insertBefore(this.createTextScript(code), firstScriptDep.node);
    }
    ...
    for (const { node, request, async, importType, inner } of deps) {
      ...
      if (!resolvedDep) {
        throw new Error(
          `Internal: missing module by request ${request} in ${asset.module.id}`
        );
      }

      const neededAssets =
        asset.manifest.moduleToAssets.get(get(hashIds, resolvedDep.id)) || [];

      if (importType === "css") {
        for (let i = 0; i < neededAssets.length; i++) {
          const relativeDest = neededAssets[i];
          if (i === neededAssets.length - 1) {
            this.treeAdapter.setAttr(
              node,
              "href",
              ctx.builderOptions.publicPath + relativeDest
            );
          } else {
            this.insertBefore(
              this.createHrefCss(ctx.builderOptions.publicPath + relativeDest),
              node
            );
          }
        }
        ...
        if (inlineAsset) {
          ...
          this.treeAdapter.insertText(
            ...
            `${ctx.dataToString(
              data
            )}\n__quase_builder__.r(${ctx.wrapInJsString(
              get(hashIds, resolvedDep.id)
            )});`
          );
        } else {
          if (async) {
            this.treeAdapter.insertText(
              ...
              `
                (function(){
                  ...
                  __quase_builder__.i(${ctx.wrapInJsString(
                    get(hashIds, resolvedDep.id)
                  )}).then(function(){
            ...
            this.treeAdapter.setAttr(
              node,
              "src",
              `data:text/javascript,__quase_builder__.r(${ctx.wrapInJsString(
                get(hashIds, resolvedDep.id)
              )});`
            );
...
export const packager: Packager = {
  pack(
    ...
    if (asset.srcs.size !== 1) {
      throw new Error(
        `${PACKAGER_NAME}: Asset "${asset.module.id}" to be generated can only have 1 source.`
      );
    }
