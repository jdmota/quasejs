import { FinalAsset, Manifest } from "./types";
...
export class Graph {
  ...
  init( builder: Builder ) {
    ...
    for ( const module of this.modules.values() ) {
      for ( const { required, splitPoint } of this.requires( module ).values() ) {
        if ( splitPoint ) {
  ...
  async dumpDotGraph( file: string ) {
    ...
    for ( const module of modulesList ) {
      for ( const { required } of this.requires( module ).values() ) {
...
export function processGraph( graph: Graph ) {
  ...
  const grow = ( from: Module ): Set<Module>|null => {
    const group = groups.get( from );
    const wouldSplitSrc = ( src: Module ) => {
