...
import { ResolvedDep, Dep, Transforms, FinalModule, ResolvedInnerDep, Loc, TransformableAsset } from "../types";
...
class SingleResolveComputation extends Computation<ResolvedDep> {
  ...
  constructor( registry: ComputationRegistry, module: Module, request: string, dep: Dep ) {
    super( registry );
    this.module = module;
  ...
  protected async run( _: ResolvedDep | null ): Promise<CValue<ResolvedDep>> {
    ...
    if ( !isAbsolute( path ) ) {
      return this.retError( `Resolution returned a non absolute path: ${path}`, loc );
    }
    ...
    const resolved = {
      ...
      path,
      async: !!dep.async,
      transforms: dep.transforms || res.transforms || this.builder.userConfig.getTransformationsForPath( path )
    };
...
export class ResolveComputation extends Computation<FinalModule> {
  ...
  private getSingleResolution( request: string, dep: Dep ) {
    ...
    if ( !c ) {
      c = new SingleResolveComputation( this.registry, this.module, request, dep );
      c.subscribe( this );
    ...
    if ( depsInfo && depsInfo.innerDependencies ) {
      for ( const [ innerId, innerDep ] of depsInfo.innerDependencies ) {
        ...
        const resolved = {
          ...
          transforms: innerDep.transforms || this.builder.userConfig.getTransformationsForType( innerDep.type )
        };
