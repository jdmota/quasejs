...
const isExportAll = (e: ExportedName): e is ExportedNameAll =>
  !!e.request && e.name === "*" && e.imported === "*";
...
class CheckedModule {
  ...
  getModule(request: string) {
    ...
    if (!m) {
      throw new Error(
        `Internal: dependency for ${JSON.stringify(request)} not found.`
      );
    }
  ...
  getAllFromExports(stack: Set<CheckedModule> = new Set()) {
    ...
    const checkExportFrom = (
      name: string,
      { request, loc }: ExportedNameAll
    ) => {
    ...
    if (namespaceConflict) {
      for (const name in exportsAllFrom) {
        this.checker.warn(
          `Re-exports '${name}' from ${exportsAllFrom[name].join(
            " and "
          )}. See ${this.id}`
        );
      }
  ...
  checkImportsExports() {
    ...
    const check = ({ request, imported, loc }: ImportedName | ExportedName) => {
      if (request && imported) {
        ...
        if (isEmpty(exports)) {
          this.error(
            `${request} exports nothing${
              imported === "*" ? "" : `. Looking for ${imported}`
            }`,
            loc
          );
        }
...
class CheckerImpl implements ICheckerImpl {
  ...
  get(id: string) {
    ...
    throw new Error(
      `Internal: checked module ${JSON.stringify(id)} not found.`
    );
  }
