...
export class PluginRegistry<T extends { name?: string; options?( flags: any ): any }> {
  ...
  private async handle(
    map: Map<string, Plugin<T>>, _name: string, plugin: T, _flags: any
    ...
    const options = plugin.options ? await plugin.options( flags ) || flags : flags;
    const obj: Plugin<T> = {
      name,
      plugin,
      options
