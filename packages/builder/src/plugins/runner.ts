...
import { TransformableAsset, GenerateOutput, FinalAsset, ToWrite, Transforms, Resolver, Transformer, Packager, Plugin } from "../types";
...
function validate( hook: string, expected: string, actual: unknown, name: string|null ) {
  error( {
    message: `'${hook}' expected ${expected}${actual ? ` but got ${typeOf( actual )}` : ""}${name ? ` on plugin ${name}` : ""}`,
    noStack: true
  } );
}
...
export class PluginsRunner {
  ...
  private async getTransformers( ctx: ModuleContext ): Promise<Plugin<Transformer>[]> {
  ...
  async pipeline( asset: TransformableAsset | null, ctx: ModuleContext ): Promise<TransformableAsset> {
    ...
    for ( const p of transformers ) {
      const o: TransformOutput = await this.transform( asset, p, previousGenerate, ctx );
      asset = o.result;
  ...
  private async transform(
    asset: TransformableAsset, { name, plugin, options }: Plugin<Transformer>, previousGenerate: GenerateFn | null, ctx: ModuleContext
    ...
    const generate = async( input: TransformableAsset ): Promise<GenerateOutput> => {
      if ( plugin.generate ) {
  ...
  private validateResolve( actual: any, name: string|null ): { path: string; transforms?: Transforms } | false {
    if ( actual === false ) {
      return actual;
    }
    ...
    throw validate( "resolve", "string | false | { path: string, transforms: string[][] }", actual, name );
  }

  async resolve( imported: string, module: ModuleContext ): Promise<{ path: string; transforms?: Transforms } | false> {
  ...
  async renderAsset( asset: FinalAsset, ctx: BuilderUtil ): Promise<ToWrite> {
    ...
    throw new Error( `Asset "${asset.module.id}" could not be rendered. Probably there is some plugin missing.` );
  }
