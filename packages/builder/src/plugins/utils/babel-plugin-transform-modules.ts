...
const templates = {
  ...
  exportsGetter: makeTemplate(
    `$0.${vars.export}($0.${vars.exports}, $1, function() { return $2; } );`
  ),
  exportsAssignment: makeTemplate(`$0.${vars.exports}.$1 = $2;`),
  exportsDefaultsAssignment: makeTemplate(
    `$0.${vars.exports}.default = { default: $1 }.default;`
  ),
...
export default ({ types: t }: any, options: any) => {
  ...
  function exitContext(path: any) {
    if (path[TOP_NODES]) {
      ...
      const topNodes = requires
        .map((entry: any) =>
          entry.onlySideEffects ? entry.varDeclOnlyEffect : entry.varDecl
        )
  ...
  function wrapInBlock(path: any) {
    ...
    if (
      context.isProgram() ||
      context.isBlockStatement() ||
      context.isSwitchCase()
    ) {
      return path;
    }
    const c = context.isIfStatement()
      ? context.get("consequent")
      : context.get("body");
  ...
  return {
    ...
    pre() {
      ...
      this.addRequire = (path: any, onlySideEffects: any) => {
        ...
        while (ctx) {
          if (ctx[REQUIRES] && ctx[REQUIRES][source]) {
            ...
            if (typeof blockHoist === "number") {
              entry.varDecl._blockHoist = entry.varDeclOnlyEffect._blockHoist = Math.max(
                blockHoist,
                entry.varDecl._blockHoist
              );
            }
            if (!onlySideEffects) {
        ...
        const uid = context.scope.generateUid(
          basename(originalSource, extname(originalSource))
        );
        ...
        const requireExpression = runTemplate("require", stringLiteral)
          .expression;

        const varDecl = t.variableDeclaration("var", [
          t.variableDeclarator(ref, requireExpression),
        ]);
        ...
        const entry = (context[REQUIRES][source] = {
          uid,
          varDecl,
          varDeclOnlyEffect,
          onlySideEffects,
      ...
      Program: {
        ...
        exit(path: any) {
          ...
          node.directives = node.directives.filter(
            ({ value }: any) => value.value !== "use strict"
          );
      ...
      ExportNamedDeclaration(path: any) {
        ...
        if (declaration) {
          if (
            ...
            this.insertOnTop(
              path,
              runTemplate("exportsGetter", t.stringLiteral(id.name), id)
            );
          } else {
      ...
      CallExpression(path: any) {
        ...
        if (node.callee.type === "Import") {
          ...
          if (arg && arg.type === "StringLiteral") {
            ...
            if (
              property.type === "Identifier" &&
              property.name === "onDependencyChange"
            ) {
