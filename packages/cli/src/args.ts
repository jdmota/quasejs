...
function missingSchema(command: string[]) {
  throw new Error(
    `Missing schema${command.length ? ` for command ${command.join(" ")}` : ""}`
  );
}
...
export function handleArgs(opts: CliOptions): ArgsInfo {
    ...
    if (commandOpts.commands) {
      ...
      if (argv.length === 0 || /^--?/.test(c)) {
        if (commandOpts.requiredCommand) {
          ...
          const examples = Object.keys(commandOpts.commands)
            .slice(0, 3)
            .join("|");
          validationError(
      ...
      if (last) {
        ...
        if (!commandInfo) {
          validationError(
            `${commandSet.value.join(" ")} is not a supported command`
          );
        }
  ...
  const flags = camelcaseKeys(yargsFlags, {
    exclude: ["--", /^\w$/],
    deep: true,
  });
