...
> {
  ...
  abstract combinations(
    closure: NFAState[]
  ): IterableIterator<[Transition, Set<NFAState>]>;
  ...
  do({
    start: oldStart,
    acceptingSet: oldAcceptingSet,
  }: NFA<NFAState>): DFA<DFAState> {
    this.closures = new Map();
...
> {
  ...
  abstract getTransitions(
    state: DFAState
  ): IterableIterator<[Transition, DFAState]>;

  abstract isDifferent(
    p: DFAState,
    q: DFAState,
    pairsTable: boolean[][]
  ): boolean;
