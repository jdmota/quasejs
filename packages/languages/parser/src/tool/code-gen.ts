...
export class CodeGenerator {
  ...
  numToComment(num: number) {
    ...
    if (this.inLexer) {
      ...
      if (num === 0) {
        ...
        comment = `/*${this.lexerTokenToString(
          this.grammar.idToNode.get(num) as LexerTokens
        )}*/`;
      }
    }
    return comment;
  ...
  genTransition(transition: Transition): string {
    ...
    if (transition instanceof RangeTransition) {
      if (transition.from === transition.to) {
        return `this.consume1(${transition.from}${this.numToComment(
          transition.from
        )});`;
      }
      return (
        `this.consume2(${transition.from}${this.numToComment(
          transition.from
    ...
    if (transition instanceof NamedTransition) {
      if (transition.multiple) {
        return `$${transition.name}.push(${this.genTransition(
          transition.subTransition
        ).slice(0, -1)});`;
      }
      return `$${transition.name}=${this.genTransition(
        transition.subTransition
      )}`;
    }
  ...
  genBoolExp(looks: RangeTransition[]) {
    return looks
      ...
      .map(look => {
        ...
        return (
          `${this.numToComment(look.from)}${look.from}<=this.current&&` +
  ...
  addIf(
    ifs: { looks: RangeTransition[]; goto: GoTo | false }[],
    look: RangeTransition | null,
    goto: GoTo | false
  ) {
  ...
  genAutomaton(
    { states, acceptingSet }: DFA<DState>,
  ...
  genTokenizer(
    ...
    const generics = this.grammar.options.typescript
      ? `<$Tokens, $Channels>`
      : "";
    ...
    if (this.grammar.options.typescript) {
      ...
      const channels = Object.keys(this.grammar.channels)
    ...
    for (const [transition] of lexerAutomaton.states[1]) {
      if (transition instanceof TokenFinalTransition) {
        throw new Error(
          `Token ${labels[transition.id]} is accepting the empty word`
        );
      }
    }
  ...
  genParser(parserRuleToAutomaton: Map<ParserRule, DFA<DState>>): string {
    ...
    const expectArgType = this.grammar.options.typescript
      ? `:number|string`
      : "";
    ...
    for (const [rule, automaton] of parserRuleToAutomaton) {
      const returnType = this.grammar.options.typescript
        ? this.grammar.typecheckDefinition(rule)
        : "";
      ...
      for (const k of rule.names.names.keys()) {
        keys.push(`${k}:$${k}`);
        names.push(
          rule.names.arrays.has(k)
            ? `$${k}=[]`
            : rule.names.optionals.has(k)
            ? `$${k}=null`
            : `$${k}`
        );
      }
    ...
    return `class Parser extends Q.Parser{\n${funcs.join("\n")}\nparse(){
      const r=this.rule${
        this.grammar.firstRule.name
      }();this.consume1(0);return r;
    }}`;
  }
  ...
  gen(
    ...
    const imports = this.grammar.options.typescript
      ? `import Q from "@quase/parser";`
      : `const Q=require("@quase/parser");`;
    const exporting = this.grammar.options.typescript
