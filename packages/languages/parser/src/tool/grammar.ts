...
export default class Grammar {
  ...
  getRule(name: string, node?: Node) {
    ...
    if (rule == null) {
      throw new Error(
        `Rule ${name} is not defined${node ? ` (${printLoc(node)})` : ""}`
      );
    }
    if (rule.modifiers.fragment) {
      throw new Error(
        `Rule ${name} is a lexer fragment${node ? ` (${printLoc(node)})` : ""}`
      );
    }
    return rule;
  }
  ...
  typecheckDefinition(
    node: ParserRule | LexerRule | StringNode | RegexpNode | Dot
  ) {
    if (node.type === "Dot") {
      return "$Tokens";
    }
    ...
    if (id == null) {
      ...
      let code;
      if (node.type === "ParserRule") {
        code = `export interface ${id} extends $Base<'${id}'> {${Array.from(
          node.names.names.entries()
        )
          .map(([name, names]) => {
            const isArray = node.names.arrays.has(name);
            const isOptional = node.names.optionals.has(name) && !isArray;

            return `${name}:${isArray ? "(" : ""}${Array.from(
              new Set(names.map(n => this.typecheck(n.item)))
            ).join("|")}${isOptional ? "|null" : ""}${isArray ? ")[]" : ""};`;
          })
          .join("")}};`;
      } else {
        const label = node.type === "LexerRule" ? node.name : node.raw;
        code = `export type ${id} = {id:${this.nodeToId.get(
          node
        )};label:${JSON.stringify(label)};image:string;};`;
      }
