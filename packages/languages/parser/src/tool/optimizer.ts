...
export class DfaMinimizer extends AbstractDfaMinimizer<DState, Transition> {
  ...
  addTransition( state: DState, transition: Transition, dest: DState ): void {
    ...
    } else if ( transition instanceof NamedTransition && transition.subTransition instanceof RuleTransition ) {
  ...
  isDifferentTransitions( p: DState, q: DState, pairsTable: boolean[][] ): boolean {
  ...
  isDifferent( p: DState, q: DState, pairsTable: boolean[][] ): boolean {
    return this.isDifferentTransitions( p, q, pairsTable ) || this.isDifferentRanges( p, q, pairsTable );
  }
