...
export class TypesInferrer implements RuleAnalyzer<Store> {
    ...
    if (!inter) {
      inter = {
        argTypes: new Map(
          rule.args.map(arg => [
            arg.arg,
            this.registry.freeNamed(
              TypePolarity.NEGATIVE,
              `${rule.name}_${arg.arg}`
            ),
          ])
        ),
        returnType: this.registry.freeNamed(TypePolarity.POSITIVE, rule.name),
      };
  ...
  select(pre: Store, node: SelectRule, post: Store) {
    ...
    this.registry.subtype(
      nonNull(objType.fields.get(node.field)), // TODO is this precise?
      // if we pick the upper bound for 'objType', then the select expr will not be precise, we might need ValueOf<>
      this.exprType(node),
      node
    );
