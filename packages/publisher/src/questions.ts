...
async function printCommitLog( opts: Options ) {
  ...
  const tagPattern = opts.git.tagPrefix.replace( git.versionRe, "*" ).replace( git.nameRe, opts.pkg.name ).replace( /\*?$/g, "*" );
  let latestHash;
  try {
    latestHash = await execStdout( "git", [ "rev-list", `--tags=${tagPattern}`, "--max-count=1" ] );
  } catch ( e ) {
    // Ignore
  }
  if ( latestHash ) {
    const result = await git.commitLogFromRevision( latestHash, opts.relativeFolder );
    ...
    if ( result ) {
      ...
      info( `Showing ${history.length} commits. For maybe more:\n${linkifyCompare( repositoryUrl, latestHash, opts.git.branch )}` );
    } else {
...
export default async function( opts: Options ) {
  ...
  console.log( `\nPublish a new version of ${colorette.bold( colorette.magenta( pkg.name ) )} ${colorette.dim( `(current: ${oldVersion})` )}\n` );

  const filter = ( input: string ) => ( version.isValidVersionInput( input ) ? version.getNewVersion( oldVersion, input ) : input );
  ...
  const prompts = [
    ...
    },
    {
      ...
      when: ( answers: Answers ) => !pkg.private && version.isPrereleaseVersion( answers.version || opts.version ) && !opts.tag,
      choices: async() => {
    ...
    },
    {
      ...
      name: "tag",
      message: "Tag",
      when: ( answers: Answers ) => !pkg.private && version.isPrereleaseVersion( answers.version || opts.version ) && !opts.tag && !answers.tag,
      validate: ( input: string ) => {
        if ( input.length === 0 ) {
    ...
    },
    {
      ...
      message: ( answers: Answers ) => {
        ...
        return `Will bump from ${colorette.cyan( oldVersion )} to ${colorette.cyan( answers.version + tagPart )}. Access: ${access}. Continue?`;
      }
    }
  ];
