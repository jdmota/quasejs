    ...
    },
    {
      ...
      task: async () => {
        const username = await npm.username({
          externalRegistry: npm.isExternalRegistry(pkg)
    ...
    },
    {
      ...
      task: () => {
        ...
        if (!version.isVersionGreater(pkg.version, opts.version)) {
          throw error(
            `New version \`${opts.version}\` should be higher than current version \`${pkg.version}\``
          );
        }
      },
    },
    {
      ...
      task: () => {
        if (!pkg.private && version.isPrereleaseVersion(opts.version)) {
          if (!opts.tag) {
            throw error(
              "You must specify a dist-tag using --tag when publishing a pre-release version. https://docs.npmjs.com/cli/dist-tag"
            );
          }
          if (opts.tag === "latest") {
            throw error(
              "It's not possible to publish pre-releases under the `latest` tag. Please specify something else, for example, `next`."
            );
          }
        }
      },
