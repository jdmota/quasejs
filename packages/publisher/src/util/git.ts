...
export function createTag(
  opts: Options & { git: GitOptions & { tagPrefix: string } }
) {
  return (
    opts.git.tagPrefix
      .replace(versionRe, opts.version)
      .replace(nameRe, opts.pkg.name) + opts.version
  );
}

export function createMessage(
  opts: Options & { git: GitOptions & { message: string } }
) {
  return opts.git.message
    .replace(versionRe, opts.version)
    .replace(nameRe, opts.pkg.name);
}

export async function hasUpstream() {
  const { stdout } = await execPromise("git", [
    "status",
    "--short",
    "--branch",
    "--porcelain=2",
...
export const verifyCurrentBranchIsMaster = async (
  opts: Options & { git: GitOptions }
) => {
...
export const isWorkingTreeClean = async () => {
  try {
    const { stdout: status } = await execPromise("git", [
      "status",
      "--porcelain",
    ]);
    if (status !== "") {
...
export const tagExistsOnRemote = async (tagName: string) => {
  try {
    const { stdout: revInfo } = await execPromise("git", [
      "rev-parse",
      "--quiet",
      "--verify",
      `refs/tags/${tagName}`,
...
export const commitLogFromRevision = async (
  revision: string,
