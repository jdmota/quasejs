...
export class CliCompiler {
  ...
  compileSchema({ types }: Schema, ctx: CliContext) {
    ...
    if (this.optionLines.length) {
      ...
      for (const line of this.optionLines) {
        ...
        helpArray.push(line.filter(Boolean).join(" "));
      }
    }
  ...
  typeToString(node: Type, ctx: CliContext): string {
    ...
    switch (node.type) {
      ...
      case "Union":
        return `${this.typeToString(node.type1, ctx)} | ${this.typeToString(
          node.type2,
          ctx
        )}`;
      case "Array":
  ...
  applyDecorators(
    node: TypeNotIdentifier,
    ctx: CliContext,
    decorators: Decorator[]
  ) {
    const sortedDecorators = decorators.sort((a, b) =>
      a.name === "description" ? 1 : b.name === "description" ? -1 : 0
    );
    ...
    for (const decorator of sortedDecorators) {
      ...
      switch (decorator.name) {
        case "alias": {
          ...
          const arr = (this.yargsOpts.alias[key] =
            this.yargsOpts.alias[key] || []);
