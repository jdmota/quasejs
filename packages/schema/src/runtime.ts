...
class Path {
  ...
  format() {
    return formatOption( `${this.toString()}${this.where ? ` [in ${this.where}]` : ""}` );
  }
...
function getSuggestion( unrecognized: string, allowedOptions: string[] ): string | undefined {
  return allowedOptions.find( option => {
    return leven( option, unrecognized ) as number < 3;
  } );
}

function createDidYouMeanMessage( unrecognized: string, allowedOptions: string[] ) {
...
function checkUnrecognized( path: Path, keys: string[], allowedOptions: string[], what?: string ) {
  ...
  if ( unrecognizedOptions.length === 1 ) {
    ...
    message = `Unrecognized ${what}${inStr} ${formatOption( unrecognized )}. ${didYouMeanMessage}`.trimRight();
  } else {
...
function assertType( path: Path, value: unknown, expectedType: string, meta?: Meta ) {
...
function assertSize( path: Path, size: number, expectedSize: number, meta?: Meta ) {
  ...
  add( path: Path, value: unknown ) {
    if ( typeof value === "object" && value != null ) {
      if ( this.set.has( value ) ) {
        throw new ValidationError( `Same reference in multiple places or circular references are not allowed (in ${path.format()})` );
      }
      this.set.add( value );
    }
  }
