...
export default class SourceMapExtractor extends SourceMapExtractorBase {
  ...
  private read(file: string): Promise<string> {
    return isUrl(file)
      ? this.fileGetter.fetch(file)
      : this.fileGetter.readFile(file);
  }

  private async _getMap(file: string): Promise<SourceMapInfoWithMap | null> {
    const headers = isUrl(file)
      ? (await this.fileGetter.fetchResponse(file)).headers
      : null;
    const url = headers
      ? headers.get("SourceMap") || headers.get("X-SourceMap")
      : null;
    let mapData;
