...
function validateHelper( metadata: any, callback: any ) {
  ...
  if ( metadata.status !== "todo" && metadata.type !== "group" && typeof callback !== "function" ) {
    return "Expected an implementation.";
  }

  if ( metadata.strict ) {
    if (
      metadata.exclusive || metadata.status === "failing" ||
      metadata.status === "todo" || metadata.status === "skipped"
    ) {
...
function createTest( metadata: TestMetadata | GroupMetadata, name: any, callback: any ) {
