import { Status, IRunReturn, GenericRunnable, IRunnableResult, ITestResult, IRunnable, ITest, TestMetadata } from "../types";
...
class ProxyImpl<R extends IRunnableResult, T extends GenericRunnable<R>> implements GenericRunnable<R> {
  test: T;
  ...
  constructor( test: T, seq: SequenceImpl<R, T>, proxyFn: ( ref: ContextRef, x: T, seq: SequenceImpl<R, T> ) => IRunReturn<R> ) {
    this.test = test;
    this.seq = seq;
...
abstract class SequenceImpl<R extends IRunnableResult, T extends GenericRunnable<R>> implements IRunnableResult, GenericRunnable<R> {
    ...
    for ( let i = 0; i < tests.length; i++ ) {
      ...
      if ( isPromise( runResult ) ) {
        ...
        if ( this.isConcurrent ) {
          ...
          const promises: PromiseLike<void>[] = [ runResult.then( this.addResult ) ];
        ...
        for ( let j = i + 1; j < tests.length; j++ ) {
          promise = promise.then( () => tests[ j ].run( context ) ).then( this.addResult );
        }
...
export class BeforeTestsAfterSequence extends SequenceImpl<IRunnableResult, IRunnable> implements IRunnableResult, IRunnable {
  ...
  addResult( result: IRunnableResult | ITestResult ) {
    ...
    const metadata = "metadata" in result ? result.metadata : { type: undefined, status: undefined };
