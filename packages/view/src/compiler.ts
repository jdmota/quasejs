...
function handleNode(result: Result, t: BabelTypes) {
  result.parts.push(t.numericLiteral(partValue(true, PART_NODE)));
}
...
const visitor = {
  JSXElement(
    result: Result,
    t: BabelTypes,
    { openingElement, children }: JSXElement
  ) {
...
function mainVisitor(t: BabelTypes, fn: Fn, path: BabelPath) {
  const { node } = path;

  const quaseViewTemplate = t.memberExpression(
    t.identifier("QuaseView"),
    t.identifier(TEMPLATE)
  );
  const quaseViewTemplateResult = t.memberExpression(
    t.identifier("QuaseView"),
    t.identifier(TEMPLATE_RESULT)
  );
  ...
  const templateDeclaration = t.variableDeclaration("const", [
    t.variableDeclarator(
      templateUID,
      t.newExpression(quaseViewTemplate, [
  ...
  path.replaceWith(
    t.newExpression(quaseViewTemplateResult, [
      templateUID,
...
export default function({ types: t }: { types: BabelTypes }) {
  return {
    ...
    visitor: {
      ...
      JSXFragment: (path: BabelPath) =>
        mainVisitor(t, visitor.JSXFragment, path),
    },
  };
}
